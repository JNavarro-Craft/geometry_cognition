...
@mcp.tool(name="create_joint")
def create_joint_tool(
    x: float, y: float, z: float, name: Optional[str] = None,
    dry_run: bool = False, confirm: bool = False,
) -> dict[str, Any]:
    """WRITE: create one joint at (x,y,z) in present units. name OPTIONAL — prefix-enforced (AI_)
    if given, autogenerated AI_J### if omitted (per-session counter; explicit name does not bump
    it). confirm=true mandatory; dry_run previews the resolved name + coords. The model must be
    buildable (open_model or new_blank_model first)."""
    return create_joint(x, y, z, name, dry_run, confirm)


@mcp.tool(name="create_joints")
def create_joints_tool(
    joints: list[dict], dry_run: bool = False, confirm: bool = False,
) -> dict[str, Any]:
    """WRITE: create many joints atomically. joints = list of {x, y, z, name?} (name optional per
    element). Stop-on-first-failure: returns applied/failed_at/not_attempted. dry_run previews ALL
    resolved names in order. confirm=true mandatory."""
    return create_joints(joints, dry_run, confirm)


@mcp.tool(name="delete_joint")
def delete_joint_tool(name: str, dry_run: bool = False, confirm: bool = False) -> dict[str, Any]:
    """WRITE (destructive): delete a joint. REFUSED with joint_has_connected_frames (listing them)
    if any frame touches it — delete those frames first (no cascade). confirm=true mandatory;
    dry_run reports whether it has connected frames."""
    return delete_joint(name, dry_run, confirm)


@mcp.tool(name="modify_joint")
def modify_joint_tool(
