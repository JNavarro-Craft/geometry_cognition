...
# --- Geometry primitives: joints (Fase 1h.2) ---------------------------------
# Hybrid naming: name optional (prefix-enforced if given, autogenerated AI_J### if not).
# Batch results reuse the canonical applied/failed_at/not_attempted shape (decisión #4).


class CreateJointRequest(BaseModel):
    """Body for POST /v1/joints. ``name`` optional: if given it must carry the bridge prefix;
    if omitted the bridge autogenerates AI_J###. Coords in the model's present units, Global
    Cartesian. ``confirm`` mandatory (modifies the model); ``dry_run`` previews."""

    x: float
    y: float
    z: float
    name: str | None = Field(None, description="Optional joint name; prefix-enforced if given")
    dry_run: bool = Field(False, description="If true, preview without creating")
    confirm: bool = Field(False, description="Must be true to create (modifies the model)")


class JointSpec(BaseModel):
    """One joint in a create_joints batch: coords + optional name."""
...
class CreateJointsRequest(BaseModel):
    """Body for POST /v1/joints/batch. ``joints`` is the list; each may carry its own name or
    be autogenerated. ``confirm`` mandatory; ``dry_run`` previews ALL resolved names."""
...
class JointCreated(BaseModel):
    """A created (or to-be-created) joint as facts: final name + coords."""
...
class CreateJointResponse(BaseModel):
    dry_run: bool
    validation_passed: bool = True
    would_apply: JointCreated | None = None
    applied: JointCreated | None = None


class BatchItemFailure(BaseModel):
    """Where a batch broke: the 0-based index, a description of the item, and the reason.
    Null in the normal flow (strict pre-validation); set only on a mid-loop OAPI surprise."""

    index: int
    item: str = Field(..., description="Readable description of the failed item")
    reason: str


class CreateJointsResponse(BaseModel):
    """Batch create. ``would_apply`` previews all resolved joints (dry-run); ``applied`` lists
    those created (real run). ``failed_at``/``not_attempted`` follow stop-on-first-failure."""

    dry_run: bool
    validation_passed: bool = True
    count: int
    would_apply: list[JointCreated] | None = None
    applied: list[JointCreated] | None = None
    failed_at: BatchItemFailure | None = None
    not_attempted: list[str] | None = None


class DeleteJointRequest(BaseModel):
    """Body for DELETE /v1/joints/{name}. ``confirm`` mandatory (destructive); ``dry_run``
    previews (reports whether the joint has connected frames)."""
...
class JointDeletion(BaseModel):
    """A joint delete as facts: the name and the frames connected to it (the delete is refused
    if this list is non-empty — the client deletes those frames first)."""

    name: str
    connected_frames: list[str] = Field(default_factory=list)


class DeleteJointResponse(BaseModel):
    dry_run: bool
    validation_passed: bool = True
    would_apply: JointDeletion | None = None
    applied: JointDeletion | None = None


class ModifyJointRequest(BaseModel):
    """Body for PATCH /v1/joints/{name}. Moves the joint to new coords. ``confirm`` mandatory
    (affects every connected frame); ``dry_run`` previews (old/new coords + affected frames)."""

    x: float
    y: float
    z: float
    dry_run: bool = Field(False, description="If true, preview without moving")
    confirm: bool = Field(False, description="Must be true (affects connected frames)")


class JointMove(BaseModel):
    """A joint move as facts: name, coords before/after, and the frames affected."""

    name: str
    previous_coords: list[float] | None = Field(None, description="[x,y,z] before (real run)")
    current_coords: list[float] | None = Field(None, description="[x,y,z] after / target")
    new_coords: list[float] | None = Field(None, description="[x,y,z] target (dry-run)")
    affected_frames: list[str] = Field(default_factory=list)


class ModifyJointResponse(BaseModel):
    dry_run: bool
    validation_passed: bool = True
